"""Public Python API for pareto-context-graph — no MCP protocol required.

Use this to integrate pareto context ranking directly into Python scripts
...
Example usage::
    ...
    cg = ParetoContextGraph("/path/to/your/repo")
...
class ParetoContextGraph:
    """Thin Python wrapper around the pareto_context_graph tool.
    ...
    def _call(self, command: str, **kwargs: Any) -> dict:
        ...
        raw = _handle_tool_call(self.repo_root, "pareto_context_graph", arguments)
    ...
    def mark_used(
        ...
        *,
        ...
        request_id:
            ``request_id`` from a prior ``context`` response (recommended).
        query:
            Optional query string echoed from the context request.
        """
        kwargs: dict[str, Any] = {"paths": paths}
        if request_id:
            kwargs["request_id"] = request_id
        if query:
            kwargs["query"] = query
        return self._call("mark_used", **kwargs)
    ...
    def feedback_accept(
        ...
        *,
        ...
        """Strong positive signal: user accepted suggested context files."""
        kwargs: dict[str, Any] = {"request_id": request_id, "paths": paths}
        if query:
            kwargs["query"] = query
        return self._call("feedback_accept", **kwargs)
    ...
    def feedback_cite(
        ...
        *,
        ...
        """Positive signal: assistant cited these files in its answer."""
        kwargs: dict[str, Any] = {"request_id": request_id, "paths": paths}
        if query:
            kwargs["query"] = query
        return self._call("feedback_cite", **kwargs)
    ...
    def feedback_reject(
        ...
        *,
        ...
        """Negative signal: returned files were not useful."""
        kwargs: dict[str, Any] = {"request_id": request_id, "paths": paths}
        if query:
            kwargs["query"] = query
        return self._call("feedback_reject", **kwargs)
    ...
    def feedback_view(
        ...
        *,
        ...
        """Weak negative signal: user opened but did not use the file."""
        kwargs: dict[str, Any] = {"request_id": request_id, "paths": paths}
        if query:
            kwargs["query"] = query
        return self._call("feedback_view", **kwargs)
    ...
    def feedback_dwell(
        ...
        *,
        ...
        """Dwell time on context files; ≥30 seconds counts as positive.

        Parameters
        ----------
        request_id:
            ``request_id`` from the ``context`` response.
        paths:
            Files the user kept open.
        dwell_seconds:
            Seconds viewed; values ≥30 are treated as strong positives.
        query:
            Optional query echoed from the context request.
        """
        kwargs: dict[str, Any] = {
        ...
        if query:
            kwargs["query"] = query
        return self._call("feedback_dwell", **kwargs)

    def learn(self, *, ranker: str = "auto") -> dict:
        """Fold feedback events and train learned weights + ranker.

        Parameters
        ----------
        ranker:
            ``auto`` (LambdaMART when lightgbm installed), ``logistic``, or
            ``lambdamart``.
        """
        from .feedback_replay import apply_learn

        return apply_learn(self.repo_root, ranker=ranker)
