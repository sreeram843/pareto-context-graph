...
_CHUNK_PATTERNS = [
    ...
    re.compile(
        r"^([ \t]*)((?:RSpec\.)?(?:shared_examples_for|shared_context|describe|context|it)\s+[^\n]*\bdo)\s*$",
        re.MULTILINE,
    ),
    ...
    re.compile(
        r"^([ \t]*)((?:export\s+)?(?:async\s+)?function\s+\w+|class\s+\w+|(?:export\s+)?(?:const|let)\s+\w+\s*=\s*(?:async\s*)?\()",
        re.MULTILINE,
    ),
    ...
    re.compile(
        r"^([ \t]*)((?:pub\s+)?(?:async\s+)?fn\s+\w+|impl\s+\w+|struct\s+\w+|trait\s+\w+)",
        re.MULTILINE,
    ),
]
...
def extract_chunks(file_path: Path) -> list[dict]:
    ...
    for i, (start_line, indent, signature) in enumerate(starts):
        ...
        name_match = re.search(
            r"(?:def|function|class|module|fn|impl|struct|trait|const|let)\s+(\w+)", signature
        )
        if name_match:
            ...
            desc_match = re.search(
                r'(?:describe|context|it|shared_examples_for|shared_context)\s+["\']([^"\']{1,60})["\']',
                signature,
            )
            if desc_match:
...
_SYMBOL_PATTERNS = [
    ...
    # C/Java
    re.compile(
        r"^[ \t]*(?:public|private|protected|static|virtual|override)?\s*\w+[\w<>*&: ]*\s+(\w+)\s*\(",
        re.MULTILINE,
    ),
]
