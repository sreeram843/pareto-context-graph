...
def export_snapshot(repo_root: Path, out_path: Path) -> Path:
    """Export .pareto-context-graph contents as a compressed tarball."""
...
def import_snapshot(repo_root: Path, in_path: Path) -> Path:
    """Import .pareto-context-graph contents from a compressed tarball."""
    if not verify_file(in_path):
        raise PermissionError(f"snapshot signature verification failed: {in_path}")
