...
from .edge_weights import EDGE_KIND_WEIGHT
...
_IMPORT_PATTERNS = [
    ...
    re.compile(
        r"""^\s*(?:const|let|var)\s.*?=\s*require\s*\(\s*['"]([^'"]+)""", re.MULTILINE
    ),  # CJS
...
def get_file_summary(file_path: Path) -> str:
    ...
    for line in lines:
        ...
        if any(
            ...
            for kw in ("class ", "module ", "def ", "function ", "export ", "pub ")
        ):
            return stripped[:120]
    return ""
...
def blast_radius(
    ...
    *,
    ...
    for depth in range(1, max_depth + 1):
        if expired_check and expired_check():
            break
        next_frontier: list[str] = []
        for src_idx, src in enumerate(frontier):
            if expired_check and expired_check():
                break
            if expired_check and deadline_tick(src_idx) and expired_check():
                break
            if use_cache:
            ...
            if use_structural:
                for dst, kind, confidence in store.structural_neighbours(src):
                    struct_weight = max(1, int(10 * EDGE_KIND_WEIGHT.get(kind, 0.5)))
                    if dst in visited:
                        if struct_weight > visited[dst]["weight"]:
                            visited[dst]["weight"] = struct_weight
                            visited[dst]["edge_kind"] = kind
                        continue
                    visited[dst] = {
                        "path": dst,
                        "depth": depth,
                        "weight": struct_weight,
                        "source": src,
                        "edge_kind": kind,
...
def blast_radius_paths(
    ...
    *,
    ...
    try:
        results = blast_radius(store, changed_files, min_weight=min_weight, max_depth=max_depth)
...
def surprise_score(results: list[dict], changed_files: list[str]) -> list[dict]:
    ...
    for r in results:
        ...
        max_dist = max(_dir_distance(r["path"], cf) for cf in changed_files) if changed_files else 0
